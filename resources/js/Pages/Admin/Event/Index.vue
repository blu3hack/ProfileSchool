...
/** Daftar agenda dengan pencarian, sakelar terbit, dan hapus. */
...
watch(search, (value) => {
    ...
    debounce = setTimeout(() => {
        router.get('/admin/event', { search: value || undefined }, {
            preserveState: true,
            replace: true,
        });
    }, 350);
...
const destroy = (item) => {
    if (!window.confirm(`Hapus agenda "${item.title}"? Tindakan ini tidak bisa dibatalkan.`)) {
        return;
    }

    router.delete(`/admin/event/${item.id}`, { preserveScroll: true });
};

const togglePublish = (item) => {
    router.patch(`/admin/event/${item.id}/terbit`, {}, { preserveScroll: true });
};
</script>

<template>
    <AdminLayout title="📅 Next Event" subtitle="Tambah, ubah, terbitkan, atau hapus agenda kegiatan sekolah.">
        <div class="mb-5 flex flex-wrap items-center gap-3">
            <Link href="/admin/event/tambah"
                class="rounded-xl bg-teal-600 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-teal-700">
                + Tambah Agenda
            </Link>

            <input v-model="search" type="search" placeholder="Cari judul, kategori, atau lokasi…"
            ...
            <span class="text-sm text-slate-500">{{ props.events.total }} agenda</span>
        </div>
        ...
        <div data-lenis-prevent class="admin-scroll overflow-x-auto rounded-2xl border border-slate-200 bg-white">
            <div v-if="!props.events.data.length" class="px-5 py-12 text-center text-sm text-slate-500">
            ...
            <table v-else class="w-full min-w-3xl text-left text-sm">
                <thead class="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
                    <tr>
                        <th class="w-20 px-4 py-3">Gambar</th>
                        <th class="px-4 py-3">Judul</th>
                        <th class="w-44 px-4 py-3">Jadwal</th>
                        <th class="w-32 px-4 py-3">Kategori</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="item in props.events.data" :key="item.id" class="hover:bg-slate-50/60">
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <Link :href="`/admin/event/${item.id}/ubah`"
                        ...
                        <td class="px-4 py-3">
                            ...
                            <span v-if="!item.is_upcoming"
                                class="mt-1 block text-[10px] font-bold uppercase tracking-wide text-slate-400">
                                Telah lewat
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a :href="`/event/${item.slug}`" target="_blank"
        ...
        <!-- Halaman -->
        <div v-if="props.events.links.length > 3" class="mt-5 flex flex-wrap gap-1">
            <component :is="link.url ? 'a' : 'span'" v-for="link in props.events.links" :key="link.label" :href="link.url"
